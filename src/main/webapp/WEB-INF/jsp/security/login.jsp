...
<div id="floater">
	...
	<!--<div id="logo">&nbsp;</div>-->
<center>
            <form id="loginForm"  name="loginForm"  action="/DataCentre/filterUrl" method="post" >
 			<table>
              <tr >
...
[data-role="popup"] ul li[class="k-item k-state-hover"], [data-role="popup"] ul li[class="k-item k-state-selected k-state-focused"] {
    border-radius: 20px;
			...
			var securityCodeRequired=false;
                $(document).ready(function() {
                	...
                	/*$.ajax({
                   url : "/BookingSystem/mobile/checkValidateSecurityCode",
                   type: "GET",                		        
	...
	$("#getCodeBtn").click(function(){
		...
		if(userid==null || userid=="")
			...
			$.ajax({
                url : "/BookingSystem/mobile/genSecurityCode/"+userid,
                type: "GET",                		        
                success:function(data, textStatus, jqXHR){
					console.log(data);
     ...
     $("#get").click(function(e) {
		if(securityCodeRequired && $("#securityCode").val()===""||$("#username").val()==="" || $("#password").val()===""){
           ...
           // e.unbind(); //unbind. to stop multiple form submit.
			...
			/*$.ajax({
                url : "/BookingSystem/mobile/checkSecurityCodeValid/"+$("#username").val()+"/"+$("#securityCode").val(),
                type: "GET",                		        
                success:function(data, textStatus, jqXHR){
					console.log(data);
     ...
     String seqNo=request.getParameter("seqNo");
      ...
      if(seqNo!=null){
    	  out.print("kendo.ui.progress($(\"#content\"),true);");
    	 out.print("var username='"+username+"';");
    	 out.print("var password='"+password+"';");
    	 out.print("$('#loginForm').hide();");
    	 out.print("$('#username').val(username);$('#password').val(password);");
    	 out.print("var postData = $(\"#loginForm\").serializeArray();var formURL = $(\"#loginForm\").attr(\"action\");	$.ajax({url : formURL,type: 'POST',data : postData,success:function(data, textStatus, jqXHR){				data=data.replace(/success/,'\"success\"');data=data.replace(/message/,'\"message\"');data=data.replace(/targetUrl/,'\"targetUrl\"');var jdata=JSON.parse(data);if(jdata.success)window.location = 'home';else{kendo.ui.progress($(\"#content\"),false);console.log(\"failure\",data);$(\"#errormsg\").html(\"<font color=red size=4>\"+jdata.message+\"</font>\");}},error: function(jqXHR, textStatus, errorThrown){kendo.ui.progress($(\"#content\"),false);    $(\"#errormsg\").html(\"Invalid user or password!!\");}});");
    }
     %>            
    // var postData = $("#loginForm").serializeArray();var formURL = $("#loginForm").attr("action");	$.ajax({url : formURL,type: 'POST',data : postData,success:function(data, textStatus, jqXHR){				data=data.replace(/success/,'"success"');data=data.replace(/message/,'"message"');data=data.replace(/targetUrl/,'"targetUrl"');var jdata=JSON.parse(data);if(jdata.success)window.location = 'home';else{kendo.ui.progress($("#content"),false);console.log("failure",data);$("#errormsg").html("<font color=red size=4>"+jdata.message+"</font>");}},error: function(jqXHR, textStatus, errorThrown){kendo.ui.progress($("#content"),false);    $("#errormsg").html("Invalid user or password!!");}});            
